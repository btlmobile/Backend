...
from src.features.api.bottle.bottle_message import (
    BOTTLE_ERROR_STATUS,
    BOTTLE_NOT_FOUND_ERROR,
    STORED_BOTTLE_ALREADY_EXISTS_ERROR,
    STORED_BOTTLE_NOT_FOUND_ERROR,
)
from src.features.api.bottle.stored_bottle_entity import StoredBottleEntity
...
class BottleService(Singleton):
    def create_bottle(self, payload, creator: str) -> tuple[BottleEntity | None, str | None]:
        try:
    ...
    def get_bottle(self, bottle_id: str) -> tuple[BottleEntity | None, str | None]:
        try:
    ...
    def list_bottles(self, page: int = 1, limit: int = 5) -> tuple[list[BottleEntity], str | None]:
        try:
            all_ids = list(BottleEntity.all_pks())
            start = (page - 1) * limit
            end = start + limit
            page_ids = all_ids[start:end]
            bottles = [BottleEntity.get(bottle_id) for bottle_id in page_ids]
            return bottles, None
        except Exception:
            return [], None

    def delete_bottle(self, bottle_id: str) -> tuple[bool, str | None]:
        bottle, error = self.get_bottle(bottle_id)
        if error:
            return False, error
        BottleEntity.delete(bottle.pk)
        return True, None
